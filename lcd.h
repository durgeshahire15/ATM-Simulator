...
void int_to_string(unsigned int,unsigned char *temp_string);//convert unsigned intto string of corresponding decimal value 

//Signals to LCD
sbit RS=P0^0;	//Register select
sbit RW=P0^1;	//Read from or write toregister
...
void lcd_init(void)
{
	...
	lcd_cmd(0x38);	// Function set: 2 Line, 8bit, 5x7 dots
	msdelay(4);
	lcd_cmd(0x06);	// Entry mode, autoincrement with no shift
	msdelay(4);
